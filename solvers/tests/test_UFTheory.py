...
f_x, f_y, f_s, f_t, f_c, f_a = [parser.parse("f(" + t + ")") for t in "xystca"]
...
def test_case1():  # (f^3(a) = a) & (f^5(a) = a) & (f(a) != a)
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints.items()}
    )
...
def test_case2():
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints_2.items()}
    )
    ...
    assert not uf_theory.check_t_propagations()
    assert uf_theory.is_t_conflict()
    assert uf_theory.analyze_satisfiability() == (ResultCode.UNSAT, {-1, -2, 4, -3})
    assert not uf_theory.pop_t_propagation()
...
def test_case3():
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints_3.items()}
    )
...
def test_case4():
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints_4.items()}
    )
...
def test_case5():
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints_5.items()}
    )
    ...
    assert uf_theory.check_t_propagations()
    assert uf_theory.is_t_conflict()
    assert uf_theory.analyze_satisfiability() == (ResultCode.UNSAT, {-1, -3, 7})
...
def test_case6():
    ...
    uf_theory.reset()
    uf_theory.register_abstraction_map(
        {v: k for (k, v) in abs_literals_to_ints_6.items()}
    )
...
@pytest.mark.parametrize("formula_str", ["[1, 2] < 5", "[1, -2] >= 3"], ids=repr)
