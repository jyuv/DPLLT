...
expected_complex_formulas = [
    Or(And(Negate(x), y), Func("f", (x, y))),
    And(a, neg_a),
    Or(a, neg_b),
    Negate(neg_b),
    Or(And(a, b), And(neg_a, Imply(c, b))),
    And(
        And(Or(Or(a, neg_b), And(r, neg_a)), Or(And(c, r), Imply(c, And(neg_r, b)))),
        Imply(And(Imply(n, a), Or(neg_r, n)), And(And(r, n), Or(a, neg_n))),
    ),
]


@pytest.mark.parametrize(
    "formula_str, expected_parsed",
...
arrays_formulas_strs = ["[5,3,-2,1] < 4", "[5,3,-2,1] >= 4", "([1, 3] < 5) & (a | b)"]

arrays_formulas = [
    Less(np.array([5, 3, -2, 1]), 4),
...
@pytest.mark.parametrize(
    "ambiguous_formula_str",
    ["a & b & c", "(!a & b)", "(a & b) -> (!(c) | f)", "(a & ()) | d"],
)
...
@pytest.mark.parametrize(
    "unbalanced_parentheses_str", ["((a)", "a & (b -> (!c)))", "((a))) & (c)", ")(a)"]
)
...
@pytest.mark.parametrize("invalid_delimiter_str", ["b,c", "f(a, v) & c,", "(,)"])
