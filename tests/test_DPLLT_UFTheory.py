...
str_uf2 = "({0}) & (({1}) & (({2}) & (({3}) & ({4}))))".format(*clauses_strs_uf2)
...
@pytest.mark.parametrize(
    "formula_text, expected_result_code",
    [
...
@pytest.mark.parametrize("formula_text", ["(a = b) & (f(c, d) = g(a))", "f(c, !d)"])
...
@pytest.mark.parametrize(
    "formula_text", ["f(c, d=c)", "f(a, f(a, g(d), a | b), c)", "f(a, !!c)"]
)
